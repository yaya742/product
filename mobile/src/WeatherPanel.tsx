        ...
        {!loading && current && weather && (
          <>
            <section className="weather-current-card">
              <div className="weather-card-heading"><div><p className="eyebrow">{copy.current}</p><h2>{selectedLocation.displayName}</h2></div><span className="weather-updated-time">{formatWeatherHour(current.time, language)}</span></div>
            ...
            <section className="weather-rain-card"><div className="weather-card-heading"><div><p className="eyebrow">{copy.hourly}</p><h2>{copy.next24}</h2></div><span className="weather-live-badge">{formatWeatherHour(current.time, language)}</span></div>{rain.length ? <div className="weather-rain-intervals">{rain.map((item) => <span key={item}>{item}</span>)}</div> : <p>{copy.noRain}</p>}</section>
            <section className="weather-forecast-card weather-hourly-card"><div className="weather-card-heading"><div><p className="eyebrow">{copy.hourly}</p><h2>{copy.hourly}</h2></div><span className="weather-hourly-legend">%</span></div><div className="weather-hourly-list">{weather.hourly.map((hour) => <div className="weather-hour" key={hour.time}><span className="weather-hour-time">{formatWeatherHour(hour.time, language)}</span><span className="weather-hour-icon">{weatherIcon(hour.weatherCode)}</span><span className="weather-hour-temp">{Math.round(hour.temperature)}°</span><span className="weather-hour-rain"><span className="weather-rain-bar"><i style={{ width: `${Math.min(100, Math.max(0, hour.precipitationProbability))}%` }} /></span><strong>{hour.precipitationProbability}%</strong></span></div>)}</div></section>
            <section className="weather-forecast-card"><div className="weather-card-heading"><div><p className="eyebrow">{copy.forecast}</p><h2>{copy.forecast}</h2></div></div><div className="weather-forecast-list">{weather.daily.map((day) => <div className="weather-day" key={day.date}><span className="weather-day-date">{formatWeatherDay(day.date, language)}</span><span className="weather-day-symbol">{weatherIcon(day.weatherCode)}</span><strong>{weatherCodeText(day.weatherCode, language)}</strong><span>{Math.round(day.min)}° / {Math.round(day.max)}°</span><small>{day.precipitationProbability}%</small></div>)}</div></section>
