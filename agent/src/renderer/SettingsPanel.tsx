...
export function SettingsPanel({ state, busy, onState, onClose, notify, onClear, draft }: Props) {
  ...
  async function perform(action: string, fn: () => Promise<void>) {
    ...
    try {
      ...
      const failure = errorText(e).replace(/[。.!！]+$/, '');
      setFeedback({
        action,
        text:
          action === 'test' && testCancelled.current
            ? '已取消测试'
            : action === 'save' && state.settings.hasKey
              ? `${failure}；此前已保存的连接未被替换`
              : failure,
            ...
            <section className="campus-source-block">
              <div className="connector connector-primary">
                <div>
                  ...
                  <p>
                    {connectorReady
                      ? '已用浙大账号连接，提问时会按需读取；这不是浙大官方授权接口'
                      : connector?.credentialsConfigured
                        ? '登录信息已安全保存，再验证一次即可使用'
                        : '使用浙大统一身份认证，不需要导入资料文件；连接仅在本机运行'}
                  </p>
                </div>
                <button
                  className="subtle-button"
              ...
              {connector?.configured && (
                <details className="connector-more">
                  ...
                  <small>
                    ...
                    这是未由浙大官方为本应用提供的本机兼容连接，接口变化或验证码可能导致读取失败。
