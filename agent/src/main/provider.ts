...
function transportFailure(code: string): ProviderError {
  if (['ENOTFOUND', 'EAI_AGAIN'].includes(code))
    return new ProviderError('无法解析 DeepSeek 地址，请检查 DNS 或代理后重试。', true, 'network');
  if (/CERT|TLS|SSL/.test(code))
    return new ProviderError('DeepSeek 安全连接校验失败，请检查系统时间、代理或安全软件。', true, 'network');
  if (['ETIMEDOUT', 'UND_ERR_CONNECT_TIMEOUT', 'UND_ERR_HEADERS_TIMEOUT'].includes(code))
    return new ProviderError('连接 DeepSeek 超时，请检查网络或代理后重试。', true, 'timeout');
  if (['ECONNRESET', 'UND_ERR_SOCKET'].includes(code))
    return new ProviderError('与 DeepSeek 的连接被中断，已自动重试但仍未完成。', true, 'network');
  return new ProviderError('无法连接 DeepSeek，请检查网络、代理或安全软件后重试。', true, 'network');
}

async function retryDelay(signal: AbortSignal, milliseconds = 350): Promise<void> {
  ...
  async complete(
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      try {
        ...
        if (signal.aborted) {
          if (signal.reason && typeof signal.reason === 'object' && 'name' in signal.reason && signal.reason.name === 'TimeoutError')
        ...
        // Only a bounded category is logged. Never include URL, headers, API
        // keys, response bodies or user content in transport diagnostics.
        console.warn('[在场] DeepSeek transport failed', JSON.stringify({ code: code || 'UNKNOWN', attempt: attempt + 1 }));
        if (attempt === 0) {
