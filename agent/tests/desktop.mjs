...
try {
  ...
  const material = await page.evaluate(() => {
    ...
    return {
      ...
      pointerX: shell?.style.getPropertyValue('--pointer-x') || '',
    };
  });
  assert.match(material.ambient, /gradient/);
  assert.match(material.composerBackdrop, /blur/);
  await page.evaluate(() => window.dispatchEvent(new PointerEvent('pointermove', { clientX: 90, clientY: 110 })));
  await page.waitForTimeout(50);
  const pointerX = await page.locator('.app-shell').evaluate((el) => el.style.getPropertyValue('--pointer-x'));
  assert.equal(pointerX, '90px');
  report.checks.push('桌面背景有可透出的环境光，输入区使用玻璃材质，指针柔光按帧更新');
