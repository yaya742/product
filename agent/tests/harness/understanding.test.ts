...
test('a bare personal campus lookup cannot become current-only or an other-person scope', async () => {
  const original = '课表呢';
  const { sources: _, ...proposal } = controlProposal({
    subject: 'other',
    sourceAllowlist: ['current'],
    sourceBasis: original,
    uncertain: true,
    uncertainControls: [{ dimension: 'sources', quote: original }],
  });
  const result = await parseTurnControls(original, {
    complete: async () => ({ content: '', tool_calls: [{ id: 'campus-short', type: 'function', function: { name: 'propose_turn_controls', arguments: JSON.stringify(proposal) } }] }),
  }, new AbortController().signal, { sources: ['current', 'history:self', 'campus:zju-account'], capabilities: [] });
  assert.equal(result.subject, 'self');
  assert.equal(result.world, 'real');
  assert.equal(result.audience, 'self');
  assert.equal(result.sources, 'unchanged');
  assert.equal(result.sourceAllowlist, null);
  assert.deepEqual(result.sourceExclusions, []);
  assert.equal(result.sourceBasis, null);
  assert.deepEqual(result.uncertainControls, []);
});
...
test('an incomplete control proposal gets at most one clean retry with unchanged thinking and limits', async () => {
  ...
  const restricted = await parseTurnControls(original, client, new AbortController().signal);
  assert.equal(restricted.sources, 'current_only');
  assert.deepEqual(restricted.sourceAllowlist, ['current']);
