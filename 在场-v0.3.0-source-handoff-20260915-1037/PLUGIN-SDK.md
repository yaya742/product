...
接口管理器支持从设置页导入 ZIP 插件包。选择文件后，宿主先验证根目录 `plugin.json`、入口文件、版本和路径安全性，再展示名称、版本、能力、权限、网络范围、平台、离线策略和许可，由用户确认后才写入插件目录。没有有效插件清单的普通 ZIP、文档压缩包或安装程序会在宿主层被拒绝；取消确认不会产生安装或升级记录。插件包不会写入源码，保存在当前用户的在场数据目录中；安装、升级、卸载完成后关闭并重新打开在场即可切换版本。天气不是宿主内置 Provider，而是仓库中的可安装示例插件，打包命令为 `npm run package:weather`，生成 `artifacts/weather-plugin.zip`。
