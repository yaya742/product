...
for (const entry of fs.readdirSync(pythonHome, { withFileTypes: true })) {
  if (!['Lib', 'DLLs', 'LICENSE.txt'].includes(entry.name) && !/\.(?:exe|dll)$/.test(entry.name)) continue;
...
const config = { ...JSON.parse(fs.readFileSync('package.json', 'utf8')).build, extraResources: [{ from: target, to: 'hermes-runtime', filter: ['**/*', '**/.*', '**/.*/**'] }] };
