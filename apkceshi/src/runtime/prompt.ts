...
function languageInstruction(language: MobileLanguage): string {
  if (language === 'en') return '请使用 English 回复用户。用户如果使用其他语言，也请优先用 English 回答。工具调用前后的可见最终回复也必须保持 English。';
  if (language === 'zh-TW') return '請使用繁體中文回覆使用者。使用者如果使用其他語言，也請優先用繁體中文回答。無論工具描述或資料欄位使用何種語言，最終可見回覆都必須保持繁體中文。';
  return '请使用简体中文回复用户。用户如果使用其他语言，也请优先用简体中文回答。无论工具描述、数据字段或内部草稿使用何种语言，最终可见回复都必须保持简体中文；不要输出英文开场或英文工具说明。';
}
