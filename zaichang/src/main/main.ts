...
async function verifyDeepSeekConnection(apiKey: string, signal: AbortSignal) {
  await createModelClient(apiKey, DEEPSEEK_MODEL).complete(
    ...
    signal,
    undefined,
    { maxOutputTokens: 256, thinking: 'disabled' },
  );
}
function state() {
  const key = getKey();
...
app.whenReady().then(async () => {
  ...
  handle('campus:account:connect', async () => {
    ...
    await zju.verifyAccount(AbortSignal.timeout(75_000));
    harness.invalidateCampusCache();
    return state();
  });
  ...
  // Campus snapshots are persisted locally and refreshed at most once every
  // two days. The adapter checks the timestamp before touching the network,
  // so this inexpensive timer does not make ordinary replies wait.
  const campusRefreshTimer = setInterval(() => {
    if (process.env.ZAICHANG_TEST !== '1') void zju.refreshIfDue();
  }, 30 * 60_000);
  void store.runtime.reminders.runDue();
  app.on('will-quit', () => {
    clearInterval(reminderTimer);
    clearInterval(recoveryTimer);
    clearInterval(campusRefreshTimer);
    store.close();
  });
});
