...
校园资料入口现在提供仓库内的 PC 本机兼容连接器 `integrations/zju-student-info`，打包版会随 Electron 一起放入 `extraResources`。在“连接与偏好 → 校园资料”中主动登录后，它只读课表、课程、考试和学在浙大待办；成绩、绩点、校园卡、预约等域会明确显示为尚未接入。账号密码由 Windows 当前用户 DPAPI 加密保管，统一认证 Cookie 只在单次连接器进程内存中存在，模型和 renderer 都拿不到密码。它不是浙江大学官方授权 API，业务接口变化或验证码可能导致读取失败；课表第一阶段保留星期和节次，未把校历/调停课猜成确定日期。校园资料按领域、来源、学校和学期合并；部分同步不清空其他领域。模型通过 `look_up` 按需读取。天气不是宿主内置能力，安装 `weather` 插件后才可查询；插件默认停用，城市参考预报不代表精确位置。

在“连接与偏好 → 接口”中可以导入第三方插件 ZIP，也可以为已安装插件选择升级或卸载。导入时在场会先验证根目录 `plugin.json`，识别名称、版本、能力、权限和网络范围，再由用户确认；普通 ZIP、文档压缩包或安装程序不会被当作插件安装。操作完成后只需关闭并重新打开在场；插件代码在独立进程运行，默认停用，权限和出站域名仍由宿主检查。插件包格式和入口约定见 [第三方插件包规范](PLUGIN-SDK.md)。

仓库自带天气插件示例。开发时运行 `npm run package:weather` 生成 `artifacts/weather-plugin.zip`，然后在设置页安装并启用；生产环境不会把天气能力偷偷作为内置 Provider 注册。

源码测试版可直接双击桌面的“在场（源码冷启动版）”快捷方式启动。该入口会在后台切换到源码目录、使用本机 Node.js 启动 Vite/Electron，并保留前端实时热更新；开发资料单独保存在 `%LOCALAPPDATA%\Zaichang\source-dev` 中，不会与已安装版抢占缓存，也不会被 Vite 纳入源码监听。源码版和便携版现在会互相查找本机两个资料目录中的加密 Key，避免切换启动入口后 Key“消失”；若两个目录里的 Key 都无法解密，仍需在当前设置页重新输入。如果默认端口被占用，会把实际端口传给 Electron；未检测到主进程源码更新时会跳过 Electron bundle 编译，缩短冷启动时间。桌面便携版入口已更新为本轮重新打包的版本，旧文件保留为日期备份。
