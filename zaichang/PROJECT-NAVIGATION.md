...
本文件是职责、当前策略、集成状态和实际文件索引的唯一维护位置。它不是学生运行时资料，也不是永久架构。检查日期：2026-09-20（Asia/Shanghai），应用 **0.3.0**。本次基线 HEAD 为 `08f95f633536fb119c5dd7c9206ccd3aa22ab81b`，工作区有大量此前未提交改动。

**本次桌面视觉/发布对照增量（2026-09-20）：** 参考 Pcelechron 1.3.1–1.3.3 的发布经验，在 `src/renderer` 增加有环境内容可透的桌面材质层、指针柔光和浅色/高对比度回退；在 `src/main/main.ts` 显式固定 Windows `appUserModelId=studio.zaichang.desktop`。没有改动现有 `appId`、资料目录、深链或手机端，避免仅因显示层调整造成通知、凭据或历史资料迁移。最终状态以真实 Electron 回归和截图查看记录为准，尚未把材质效果解释为用户研究结论。
...
| 校园资料 | 按域/source/institution/term 合并，full/partial/failed/冲突；manifest 限定的 zju 本机兼容读取器桥 | 连接器离线协议/DPAPI/规范化缓存/主进程测试与构建通过；未使用真实学生账号，真实网络同步 smoke 仍待用户主动操作 |
...
| 源码冷启动 | 桌面快捷方式已改用隐藏的 `scripts/start-dev.vbs`，由 `scripts/start-dev.cmd` 保留手动诊断入口；启动器使用本机 Node.js，Vite 监听 5173（被占用时传递实际端口），Electron 通过 `ZAICHANG_DEV_SERVER_URL` 加载，源码资料写入 `%LOCALAPPDATA%\Zaichang\source-dev` | 2026-09-19 已重写桌面“在场（源码冷启动版）.lnk”，目标为当前 `start-dev.vbs`、工作目录为当前项目；从快捷方式实际启动 `scripts/dev.mjs` 与 Electron，进程使用 `source-dev` 隔离目录。 |
...
| [src/main/zjuAdapter.ts](src/main/zjuAdapter.ts) | 浙大本机兼容连接器的受限子进程桥、清单校验、按领域读取、刷新与连接状态 | 本轮修订；支持域由 connector.json 限定 |
| [integrations/zju-student-info/](integrations/zju-student-info/) | PC 端独立只读连接器：DPAPI 凭据、CAS 统一认证、教务网课表/考试、学在浙大待办、加密规范化缓存 | 本轮新增；MIT；本机兼容连接，不是浙大官方授权接口 |
